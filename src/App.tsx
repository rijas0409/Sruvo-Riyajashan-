...
export default function App() {
  ...
  const [currentPage, setCurrentPage] = useState<'home' | 'how-it-works' | 'features' | 'for-vets' | 'early-access' | 'contact' | 'privacy-policy'>('home');
  ...
  if (currentPage === 'early-access') {
    return (
      <EarlyAccess onNavigate={setCurrentPage} />
    );
  }
  ...
  return (
    <div className="bg-surface font-sans text-on-surface selection:bg-primary-container selection:text-on-primary-container min-h-screen relative overflow-x-hidden">
      ...
      {/* Bottom Layout */}
      <BottomLayout onNavigate={setCurrentPage} />
    </div>
  );
}
